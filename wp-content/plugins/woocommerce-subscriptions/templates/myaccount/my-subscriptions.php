...
<div class="woocommerce_account_subscriptions">
	...
	<h2 class="text-center title-line">
		<span>
			<?php _e( 'My Subscriptions', 'woocommerce-subscriptions' ); ?>
		</span>
	</h2>
		...
		<?php foreach ( array_reverse( $subscriptions ) as $subscription_key => $subscription_details ) : ?>
			...
			<tr class="order">
				...
				<td class="subscription-title" data-title="<?php _e( 'Subscription', 'woocommerce-subscriptions' ); ?>">
					...
					<?php endif; ?>
						<strong><?php echo WC_Subscriptions_Order::get_item_name( $subscription_details['order_id'], $subscription_details['product_id'] ); ?></strong>
	...
	<?php else : ?>
		...
		<p class="no_subscriptions"><?php printf( __( 'You have no active subscriptions. Find your first subscription in the %sstore%s.', 'woocommerce-subscriptions' ), '<a href="' . apply_filters( 'woocommerce_subscriptions_message_store_url', get_permalink( woocommerce_get_page_id( 'shop' ) ) ) . '">', '</a>' ); ?> <a class="button wc-forward" href="http://htcvr.es/sandbox/crea-tu-caja/">Adquirir Suscripción</a></p>
	<?php endif; ?>
