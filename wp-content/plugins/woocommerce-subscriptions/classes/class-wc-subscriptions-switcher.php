	...
	public static function maybe_set_apporitioned_totals( $total ) {
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( in_array( $apportion_recurring_price, array( 'yes', 'yes-upgrade' ) ) || ( in_array( $apportion_recurring_price, array( 'virtual', 'virtual-upgrade' ) ) && $is_virtual_product ) ) {
				...
				$old_recurring_total = WC_Subscriptions_Order::get_item_meta( $old_order, '_recurring_line_total', $old_subscription['product_id'], 0 ) / WC_Subscriptions_Order::get_item_meta( $old_order, '_qty', $old_subscription['product_id'], 0 );
				...
				if ( $item_data->subscription_period == $old_subscription['period'] && $item_data->subscription_period_interval == $old_subscription['interval'] ) {
					...
					$new_price_per_day = $item_data->price / $days_in_old_cycle; // We need to use the cart items price to ensure we include extras added by extensions like Product Add-ons
					...
					$new_price_per_day = $item_data->price / $days_in_new_cycle;
