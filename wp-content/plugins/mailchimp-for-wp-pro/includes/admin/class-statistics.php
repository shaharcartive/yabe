...
class MC4WP_Statistics {
	...
	*/
	...
	public function get_total_day_counts() {
		...
		$query = "SELECT COUNT(id) AS count, datetime, DATE_FORMAT(datetime, '{$this->get_date_format()}') AS date_group FROM {$this->table_name} WHERE `success` = 1 AND UNIX_TIMESTAMP(datetime) >= {$this->start_date} AND UNIX_TIMESTAMP(datetime) <= {$this->end_date} GROUP BY date_group";
	...
	public function get_checkbox_day_counts() {
		...
		$query = "SELECT COUNT(id) AS count, datetime, DATE_FORMAT(datetime, '{$this->get_date_format()}') AS date_group FROM {$this->table_name} WHERE `success` = 1 AND method = 'checkbox' AND UNIX_TIMESTAMP(datetime) >= {$this->start_date} AND UNIX_TIMESTAMP(datetime) <= {$this->end_date} GROUP BY date_group";
		$totals = $wpdb->get_results( $query );
	...
	public function get_form_day_counts() {
		...
		// get all form ids
		$query = "SELECT COUNT(id) AS count, datetime, DATE_FORMAT(datetime, '{$this->get_date_format()}') AS date_group FROM {$this->table_name} WHERE `success` = 1 AND method = 'form' AND UNIX_TIMESTAMP(datetime) >= {$this->start_date} AND UNIX_TIMESTAMP(datetime) <= {$this->end_date} GROUP BY date_group";
		$totals = $wpdb->get_results( $query );
		return $this->get_day_counts( $totals );
	}
	...
	public function get_day_counts_for_form( $form_id ) {
		...
		// get all form ids
		$query = "SELECT COUNT(id) AS count, datetime, DATE_FORMAT(datetime, '{$this->get_date_format()}') AS date_group FROM {$this->table_name} WHERE related_object_ID = $form_id AND `success` = 1 AND method = 'form' AND UNIX_TIMESTAMP(datetime) >= {$this->start_date} AND UNIX_TIMESTAMP(datetime) <= {$this->end_date} GROUP BY date_group";
		$totals = $wpdb->get_results( $query );
		return $this->get_day_counts( $totals );
	}
