...
class FUE_Addon_Woocommerce_Mailer {
    ...
    private function register_hooks() {
        ...
        // check for status changes before sending
        add_filter( 'fue_skip_email_sending', array($this, 'skip_sending_if_order_status_changed'), 10, 3 );
    }
    ...
    private function get_replacement_data( $email_data, $queue_item, $email ) {
        ...
        if ( $queue_item->is_cart ) {
            $items      = $this->fue_wc->get_persistent_cart( $queue_item->user_id );
        ...
        if ( $queue_item->order_id > 0 ) {
            $order      = WC_FUE_Compatibility::wc_get_order( $queue_item->order_id );
            $order_id   = apply_filters( 'woocommerce_order_number', $queue_item->order_id, $order );

            $replacements['order_date']             = date(get_option('date_format'), strtotime($order->order_date));
            $replacements['order_datetime']         = date(get_option('date_format') .' '. get_option('time_format'), strtotime($order->order_date));
            ...
            $replacements['dollars_spent_order']    = $this->get_plain_order_total( $order );
        }
        ...
        if ( !empty( $items ) ) {
            foreach ( $items as $item ) {
                ...
                $item_url   = FUE_Sending_Mailer::create_email_url( $queue_item->id, $email->id, $email_data['user_id'], $email_data['email_to'], get_permalink( $item_id ) );
                $product_name  = FUE_Addon_Woocommerce::get_product_name( $item_id );
                $items_array[] = $product_name;
                $item_list .= apply_filters(
                    'fue_email_item_list',
                    '<li><a href="'. $item_url .'">'. $product_name .'</a></li>',
                    $queue_item->id,
                    $item
                );
    ...
    private function get_test_replacement_data( $email_data, $email ) {
        ...
        if ( $email_type == 'storewide' ) {
            ...
            if (! empty($order_id) ) {
                ...
                $order_id = '798';
                $order_total = function_exists('wc_price') ? wc_price(121.40) : woocommerce_price(121.40);
     ...
     * @return bool
     */
    public function skip_sending_if_order_status_changed( $skip, $email, $queue_item ) {
        ...
        if ( $queue_item->order_id > 0 ) {
            ...
            if (
                in_array( $email->trigger, $statuses ) &&
                WC_FUE_Compatibility::get_order_status( $order ) != $email->trigger &&
                !FUE_Addon_Woocommerce_Conditions::order_matches_status_condition( $order, $email )
            ) {
