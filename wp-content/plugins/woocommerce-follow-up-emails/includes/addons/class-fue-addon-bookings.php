...
class FUE_Addon_Bookings {
    ...
    public function register_email_type( $types ) {
        ...
        $props = array(
            ...
            'long_description'      => __('WooCommerce Bookings is an extension that allows you to sell your time or date based bookings. You can send follow-up emails to customers that book appointments, services or rentals. <a href="admin.php?page=followup-emails-settings&tab=documentation">Learn More</a>', 'follow_up_emails'),
            'short_description'     => __('You can send follow-up emails to customers that book appointments, services or rentals.', 'follow_up_emails')
        );
    ...
    protected function add_test_variable_replacements( $variables, $email_data, $email ) {
        $variables['item_name']                 ='Booking Schedule Test';
    ...
    public function maybe_trigger_status_update( $post_id ) {
        ...
        if ( $last_status != $booking_status ) {
            update_post_meta( $post_id, '_last_status', $booking_status );
    ...
    private function create_email_order( $booking_id, $triggers = array() ) {
        ...
        $emails     = fue_get_emails( 'wc_bookings', FUE_Email::STATUS_ACTIVE, array(
            'meta_query'    => array(
                array(
                    'key'       => '_interval_type',
                    'value'     => $triggers,
