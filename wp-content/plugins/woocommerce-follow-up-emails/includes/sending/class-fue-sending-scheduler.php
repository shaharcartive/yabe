...
class FUE_Sending_Scheduler {
    ...
    public static function queue_manual_emails( $args = array() ) {
        ...
        foreach ( $recipients as $recipient ) {
            ...
            FUE_Sending_Scheduler::queue_email( $insert, $email, $schedule_emails );
            ...
            if ( $send_again && !empty($interval) && $interval > 0 ) {
                ...
                FUE_Sending_Scheduler::queue_email( $email_data, $email, true );
    ...
    public function reschedule_daily_summary_email() {
        $items = $this->get_items( array('meta' => 'daily_summary', 'is_sent' => 0) );
    ...
    private function insert_email_order( $data, $schedule_event = true ) {
        ...
        if ( empty( $email->message ) ) {
            return new WP_Error( 'fue_insert_email_order', __('Cannot add to queue an email with a missing message', 'follow_up_emails') );
        }
