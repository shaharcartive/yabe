		...
		switch ( $column_name ) {
			...
			case 'customer':
				...
				if ( $customer_email ) {
					...
					$column_content = sprintf( '<a href="%s">%s</a>', get_edit_user_link( $user_points->user_id ), ( $user ) ? $user->user_login : __( 'Unknown', 'woocommerce-points-and-rewards' ) );
				}
			...
			case 'update':
				$column_content = '<input type="text" class="points_balance" name="points_balance[' . $user_points->user_id . ']" value="' . $user_points->points_balance . '" />' .
					' <a class="button update_points" href="' . wp_nonce_url( remove_query_arg( 'points_balance', add_query_arg( array( 'action' => 'update', 'user_id' => $user_points->user_id ) ) ), 'wc_points_rewards_update' ) . '">' . __( 'Update', 'woocommerce-points-and-rewards' ) . '</a>';
			break;
